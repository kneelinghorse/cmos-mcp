...
export const cmosMessageSchema = z
  .object({
    action: z
      ...
      .describe('Message action: send | list | respond | ack | directory | whoami'),
    // send params
    targetAddress: z
      .string()
      .optional()
    ...
    messageId: z
      .string()
      .optional()
      .describe('UUID of the message to respond to (respond) or acknowledge (ack)'),
    respondStatus: z
  ...
  inputSchema: {
    ...
    properties: {
      action: {
        type: 'string',
        enum: [...CMOS_MESSAGE_ACTIONS],
        description: 'Message action: send | list | respond | ack | directory | whoami',
      },
      targetAddress: {
        type: 'string',
        description:
