// ABOUTME: Layer 3 working retrieval. FTS5Retriever (sync, BM25 × recency) is the legacy backend;
// HybridRetriever (async, BM25 + sqlite-vec cosine via RRF k=60, recency applied after fusion) is the
// Sprint 66 m04 backend covering decisions, learnings, and missions per cmos/planning/adr/s66-vector-retrieval.md.
...
/** Pluggable retrieval interface. All sync retriever backends implement this. */
export interface IRetriever {
  /**
   * Search for content relevant to the query string.
   * Returns ranked results ordered by combined score (descending).
   */
  search(query: string, options?: RetrievalOptions): RankedResult[];

  /**
   * Fetch a specific item by ID and type.
   * Returns null if not found.
   */
  get(id: number, type: RankedResultType): RankedResult | null;

  /** What this retriever supports. */
...
/**
 * FTS5-based retriever using SQLite's built-in full-text search.
 *
 * Scores results using BM25 × recency decay:
 *   score = bm25Score * (1 - w + w * exp(-ageDays / halfLife))
 * where w = recencyWeight (0 = pure BM25, 1 = full recency decay).
 *
 * Currently supports 'decision' type only. Other types (learning, mission, session)
 * can be added in future sprints via the IRetriever interface.
 */
export class FTS5Retriever implements IRetriever {
  constructor(private readonly client: CmosDatabaseClient) {}
  ...
  search(query: string, options: RetrievalOptions = {}): RankedResult[] {
    ...
    for (const type of types) {
      if (type === 'decision') {
        const decisionResults = this.searchDecisions(
          query,
      ...
      // Future: learning, mission, session backends added here
    }
  ...
  get(id: number, type: RankedResultType): RankedResult | null {
    ...
    const result = this.client.getOne<{
      id: number;
    ...
    const ageDays = computeAgeDays(row.created_at);
    ...
    return {
      ...
      score: recencyFactor, // No BM25 for direct get
      bm25Score: 1.0,
      recencyFactor,
      ageDays,
  ...
  private searchDecisions(
    ...
    // Try FTS5 first
    const ftsResults = this.tryFts5(keywords, candidateLimit);
    if (ftsResults !== null) {
      return this.scoreDecisionRows(ftsResults, recencyWeight, statusFilter);
    }

    // Keyword LIKE fallback
    return this.keywordLikeFallback(keywords, candidateLimit, recencyWeight, statusFilter);
  }

  private tryFts5(
    keywords: string[],
    candidateLimit: number
  ): Array<{ rowid: number; rank: number }> | null {
    // Check FTS5 table exists
    const check = this.client.getOne<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='decisions_fts'",
      []
    );
    if (!check.success || !check.data) return null;

    // Build OR query with quoted literals to handle special chars
    ...
    try {
      const result = this.client.getMany<{ rowid: number; rank: number }>(
        'SELECT rowid, rank FROM decisions_fts WHERE decisions_fts MATCH ? ORDER BY rank LIMIT ?',
  ...
  private scoreDecisionRows(
    ftsRows: Array<{ rowid: number; rank: number }>,
    recencyWeight: number,
    statusFilter: string[]
  ): RankedResult[] {
    const rowIds = ftsRows.map((r) => r.rowid);
    // Build a rowid → rank map for scoring after DB fetch
    ...
    return result.data.map((row) => {
      const rawRank = rankMap.get(row.id) ?? -1;
      // FTS5 rank is negative: more negative = better match
      // Convert to positive: higher bm25Score = better match
      const bm25Score = Math.abs(rawRank);
      const ageDays = computeAgeDays(row.created_at);
      const recencyFactor = computeRecencyFactor(ageDays);
      const score = bm25Score * (1 - recencyWeight + recencyWeight * recencyFactor);

      return {
        id: row.id,
        type: 'decision' as RankedResultType,
        text: row.decision_text,
        score,
        bm25Score,
        recencyFactor,
  ...
  private keywordLikeFallback(
    ...
    const likeConditions = topKeywords.map(() => 'LOWER(decision_text) LIKE ?');
    const likeParams = topKeywords.map((k) => `%${k.toLowerCase()}%`);
    const statusPlaceholders = statusFilter.map(() => '?').join(', ');
    ...
    const result = this.client.getMany<{
      ...
      `SELECT id, decision_text, category, sprint_id, evidence, created_at
       ...
       LIMIT ?`,
      [...statusFilter, ...likeParams, candidateLimit]
    );

    if (!result.success || !result.data) return [];

    return result.data.map((row) => {
      // No BM25 in fallback — use keyword overlap as proxy
      const matchCount = topKeywords.filter((k) =>
        row.decision_text.toLowerCase().includes(k)
      ).length;
      const bm25Score = matchCount / topKeywords.length; // Normalize 0–1
      const ageDays = computeAgeDays(row.created_at);
      const recencyFactor = computeRecencyFactor(ageDays);
      const score = bm25Score * (1 - recencyWeight + recencyWeight * recencyFactor);
